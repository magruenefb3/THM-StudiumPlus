...
# --- Liste der Mandanten-IDs ---
# Diese Liste wird für die Zuweisung zu den Adressen verwendet.
mandanten_ids = [
    "M-0001", "M-0002", "M-0003", "M-0004", "M-0005",
    "M-0006", "M-0007", "M-0008", "M-0009", "M-0010",
    "M-0011", "M-0012", "M-0013", "M-0014", "M-0015",
    "M-0016", "M-0017", "M-0018", "M-0019", "M-0020"
]
...
if __name__ == "__main__":
    ...
    # Liste der verfügbaren Mandanten-IDs, die gemischt und dann einmalig zugewiesen werden
    available_mandanten_ids = list(mandanten_ids) # Eine Kopie der Liste erstellen
    random.shuffle(available_mandanten_ids) # Die Reihenfolge zufällig mischen
    ...
    with open(output_csv_filename, 'w', newline='', encoding='utf-8') as outfile:
        ...
        csv_writer.writerow([
            "Adress-ID", "Mandanten-ID", "Straße", "Hausnummer", "Stadtteil", "Postleitzahl", "Stadt", "Bundesland", "Breitengrad", "Längengrad"
        ])
        ...
        while found_addresses_count < num_addresses_to_generate:
            ...
            try:
                ...
                if location:
                    ...
                    # Sicherstellen, dass die Hausnummer nie leer ist: Fallback auf original
                    geocoded_hausnummer = address_raw.get('house_number', '')
                    if not geocoded_hausnummer:
                        geocoded_hausnummer = hausnummer_orig # Fallback zur Original-Hausnummer
            ...
            if pruefergebnis_status == "Gefunden":
                ...
                # Mandanten-ID zuweisen: einmalig für die ersten Adressen, dann leer
                if found_addresses_count < len(available_mandanten_ids):
                    mandanten_id = available_mandanten_ids[found_addresses_count]
                else:
                    mandanten_id = "" # Feld leer lassen, wenn alle Mandanten-IDs zugewiesen wurden

                csv_writer.writerow([
                    address_id, # Adress-ID hinzufügen
                    mandanten_id, # Mandanten-ID hinzufügen
                    geocoded_strasse, 
                    geocoded_hausnummer, 
                    geocoded_stadtteil, 
                    geocoded_plz, 
                ...
                print(f"  -> Datensatz {address_id} (Mandant: {mandanten_id if mandanten_id else 'Nicht zugewiesen'}) in '{output_csv_filename}' geschrieben: {geocoded_strasse} {geocoded_hausnummer}, {geocoded_plz} {geocoded_stadt}, {geocoded_bundesland}")
