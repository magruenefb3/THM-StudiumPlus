...
if __name__ == "__main__":
    ...
    with open(output_csv_filename, 'w', newline='', encoding='utf-8') as outfile:
        ...
        csv_writer.writerow([
            "Straße", "Hausnummer", "Stadtteil", "Postleitzahl", "Stadt"
        ])

        print(f"Generiere und prüfe {num_addresses_to_generate} Adressen...\n")

        found_addresses_count = 0
        for i in range(num_addresses_to_generate):
            ...
            try:
                ...
                # HIER WURDE addressdetails=True HINZUGEFÜGT!
                ...
                if location:
                    ...
                    # --- WICHTIGE DEBUG-AUSGABE: Zeigt den gesamten Roh-JSON-Inhalt an ---
                    print(f"  DEBUG: Komplette Rohdaten von Nominatim: {location.raw}") 

                    # --- NEUE DEBUG-AUSGABE: Zeigt nur den 'address' Teil der Rohdaten an ---
                ...
                print(f"[{i+1}/{num_addresses_to_generate}] Prüfe: '{vollstaendige_adresse_query}' -> Status: {pruefergebnis_status}")
                
                if pruefergebnis_status == "Gefunden":
                    print(f"  Gefunden: Straße='{geocoded_strasse}', Hausnr='{geocoded_hausnummer}', Stadtteil='{geocoded_stadtteil}', PLZ='{geocoded_plz}', Stadt='{geocoded_stadt}'")


            except GeocoderTimedOut:
                pruefergebnis_status = "Timeout"
                print(f"[{i+1}/{num_addresses_to_generate}] -> Timeout bei der Prüfung von: '{vollstaendige_adresse_query}'")
            except GeocoderServiceError as e:
                pruefergebnis_status = f"Dienstfehler: {e}"
                print(f"[{i+1}/{num_addresses_to_generate}] -> Dienstfehler bei der Prüfung von: '{vollstaendige_adresse_query}' ({e})")
            except Exception as e:
                pruefergebnis_status = f"Unerwarteter Fehler: {e}"
                print(f"[{i+1}/{num_addresses_to_generate}] -> Ein unerwarteter Fehler ist aufgetreten: {e}")
            
            # Nur schreiben, wenn das Prüfergebnis "Gefunden" ist
            if pruefergebnis_status == "Gefunden":
                csv_writer.writerow([
                    geocoded_strasse, 
                    geocoded_hausnummer, 
                ...
                print(f"  -> Datensatz in '{output_csv_filename}' geschrieben: {geocoded_strasse} {geocoded_hausnummer}, {geocoded_plz} {geocoded_stadt}")
